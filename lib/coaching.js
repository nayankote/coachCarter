...
Lead with the key numbers and what they mean. Call out 1–2 high-impact actionables for next time. Be blunt. If compliance < 70, include one concrete plan adjustment.`;
...
Then, if proposal_needed is true, write the full proposal including:
1. What's changing and why
2. Why this is a plan problem, not an execution problem
3. Full week view (before and after)
4. Constraint check (recovery gaps, intensity sequencing, sport balance)
5. Reversibility — when to re-evaluate
...
async function generatePlanProposal({ trigger, feedback, context }) {
  const client = getClient();
...
Generate a plan change proposal. You MUST include:
1. WHAT'S CHANGING — specific sessions/exercises affected
2. WHY — the feedback or pattern that triggered this, and why it's a plan problem not an execution problem
3. FULL WEEK VIEW (BEFORE) — text table of current week layout
4. FULL WEEK VIEW (AFTER) — same format with changes highlighted
5. CONSTRAINT CHECK — recovery gaps, intensity sequencing, sport balance, tradeoffs
6. REVERSIBILITY — when/how this change gets re-evaluated
