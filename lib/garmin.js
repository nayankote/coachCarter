...
function deduplicateBikes(activities) {
  ...
  for (const group of Object.values(byDate)) {
    ...
    if (zwift) {
      keep.push(zwift);
      dropped.push(...group.filter(a => a !== zwift));
  ...
  return { keep: [...keep, ...others], duplicates: dropped };
}

// Fetches sleep, body battery, and stress data for a given date string (YYYY-MM-DD).
// Returns a normalized object matching the daily_metrics table columns.
async function getSleepAndWellness(client, dateStr) {
  const date = new Date(dateStr + 'T12:00:00Z'); // noon UTC to avoid timezone edge cases

  let sleepData;
  try {
    sleepData = await client.getSleepData(date);
  } catch (err) {
    console.warn(`[garmin] No sleep data for ${dateStr}: ${err.message}`);
    sleepData = null;
  }

  await sleep(1000);

  // Stress data via raw API — garmin-connect doesn't expose a dedicated method
  let stressAvg = null;
  try {
    const stressData = await client.get(
      `https://connect.garmin.com/modern/proxy/wellness-service/wellness/dailyStress/${dateStr}`
    );
    if (stressData && typeof stressData.overallStressLevel === 'number') {
      stressAvg = stressData.overallStressLevel;
    } else if (stressData && typeof stressData.avgStressLevel === 'number') {
      stressAvg = stressData.avgStressLevel;
    }
  } catch (err) {
    console.warn(`[garmin] No stress data for ${dateStr}: ${err.message}`);
  }
  ...
  return {
    ...
    sleep_hours: dto?.sleepTimeSeconds != null ? +(dto.sleepTimeSeconds / 3600).toFixed(1) : null,
    ...
    body_battery_start: bbArray.length ? bbArray[bbArray.length - 1].value : null, // morning value (end of sleep)
    body_battery_end: bbArray.length ? bbArray[0].value : null, // evening value (start of sleep)
    stress_avg: stressAvg,
  };
}

module.exports = { createGarminClient, getNewActivities, downloadFitFile, resolveSource, deduplicateBikes, getSleepAndWellness };
