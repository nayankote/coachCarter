...
function fitSportToSport(fitSport = '') {
...
// Returns per-session summary for multi_sport FIT files
async function getMultiSportSessions(buffer) {
  const data = await parseFit(buffer);
  return (data.sessions || []).map((s, i) => ({
    index: i,
    sport: fitSportToSport(s.sport),
    duration_min: s.total_elapsed_time ? +(s.total_elapsed_time / 60).toFixed(1) : null,
    start_time: s.start_time || null,
    distance_km: s.total_distance ? +s.total_distance.toFixed(2) : null,
  }));
}

// athlete = { ftp, run_threshold_sec_per_km } — passed from plan.json at call site
// sessionIndex: which session to extract (0 for regular activities; for multi_sport
//   the FIT file has multiple sessions, and extractMetrics picks the right one)
async function extractMetrics(buffer, sport, athlete = {}, sessionIndex = 0) {
  const data = await parseFit(buffer);
  // For multi_sport FIT files use sessionIndex; single-session files always use [0]
  const sessionCount = data.sessions?.length ?? 1;
  const idx = sessionCount > 1 ? sessionIndex : 0;
  ...
  if (sport === 'run') {
    ...
    const speedRecords = records.map(r => r.enhanced_speed ?? r.speed).filter(Boolean);
...
module.exports = { parseFit, extractMetrics, getMultiSportSessions, fitSportToSport };
